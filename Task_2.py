...
def _parse_hh(vacancy):
    ...
    while page <= max_page:
        for vacancy in vacancies:
            ...
            # название вакансии
            vacancy_name = vacancies.find('a', {'data-qa': 'vacancy-serp__vacancy-title'}).getText().replace(u'\xa0',
                                                                                                             u' ')
            vacancy_info['vacancy_name'] = vacancy_name

            # заработная плата
            salary = vacancies.find('span', {'data-qa': 'vacancy-serp__vacancy-compensation'})
            if not salary:
                salary_min = None
                salary_max = None
            ...
            # ссылка на вакансию
            vacancy_link = vacancies.find('a', {'data-qa': 'vacancy-serp__vacancy-title'})['href']
